...
while ($rd=readdir($od)){
if (!ereg('^\.',$rd) && filectime(H.'sys/tmp/'.$rd)<$time-60*60*24){@delete_dir(H.'sys/tmp/'.$rd);}}
closedir($od);}}
...
$ank2['level']=999;
$ank2['pol']=1;
$ank2['group_name']='Robot system';
$ank2['ank_o_sebe']='Notification';
...
$users[$user_id]['group_name']='Users';
}
else
{
...
if ($time_p[0]==date("j n Y", time()-60*60*(24-$user['set_timesdvig'])))$timep="Yesterday $time_p[1]";}
else{
if ($time_p[0]==date("j n Y"))$timep=date("H:i:s", $time);
if ($time_p[0]==date("j n Y", time()-60*60*24))$timep="Yesterday $time_p[1]";}
...
function userBirthday() {
   $sql = "SELECT id, nick  FROM user where ank_m_r = month(curdate()) and ank_d_r = dayofmonth(curdate()) ORDER BY RAND LIMIT 1"; 
   $items = mysql_query($sql); 
    while ($item = mysql_fetch_array($items)) 
    { 
      //$uage = getage($item[2]); 
      $lnk = "<a href=\"info.php?id=$item[0]\">$item[1]</a>"; 
      return "$lnk<br/>"; 
    }
}
